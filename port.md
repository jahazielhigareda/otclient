...
| T47 | Protocol | World-light, magic-effect, animated-text, distance-missile, creature-light, player-info, player-cancel-attack, walk-wait parsers | Medium | 2 | ✅ Completed | `ParseWorldLight(0x82→U8+U8)`, `ParseMagicEffect(0x83→pos+loop)`, `ParseAnimatedText(0x84→pos+U8+str)`, `ParseDistanceMissile(0x85→2×pos+U16)`, `ParseCreatureLight(0x8D→U32+U8+U8)`, `ParsePlayerInfo(0x9F→U8+U32+U8+U8+U16+spells+U8)`, `ParsePlayerCancelAttack(0xA3→U32)`, `ParseWalkWait(0xB6→U16)`. `LocalPlayer.Spells`+`SetSpells` added. `ConnectProtocol` wires `PlayerInfoReceived→IsPremium/Vocation/SetSpells`. 20 new tests. |
| T48 | Protocol | Bug-report, trappers, close-forge-window, resting-area, unjustified-stats, tutorial-hint, automap-flag, channel-event parsers | Medium | 2 | ✅ Completed | `ParseBugReport(0x1A→bool)`, `ParseTrappers(0x87→U8+U32[])`, `ParseCloseForgeWindow(0x89→no payload)`, `ParseRestingAreaState(0xA9→U8+U8+str)`, `ParseUnjustifiedStats(0xB7→7×U8)`, `ParseTutorialHint(0xDC→U8)`, `ParseAutomapFlag(0xDD→pos+U8+str+U8)`, `ParseChannelEvent(0xF3→U16+str+U8)`. `UnjustifiedStats` sealed record added. `Game.UnjustifiedStats?` property. `ConnectProtocol` wires BugReportReceived→CanReportBugs and UnjustifiedStatsReceived→UnjustifiedStats. 15 new tests. |
| T49 | Protocol | SetStoreDeepLink, ChangeMapAwareRange, DailyRewardCollectionState, OpenRewardWall, DailyReward, RewardHistory, LootContainers parsers | Medium | 2 | ✅ Completed | `ParseSetStoreDeepLink(0xA8→U8)`, `ParseChangeMapAwareRange(0x33→U8+U8)`, `ParseDailyRewardCollectionState(0xDE→U8)`, `ParseOpenRewardWall(0xE2→2-branch)`, `ParseDailyReward(0xE4→DailyRewardData)`, `ParseRewardHistory(0xE5→list)`, `ParseLootContainers(0xC0→bool+list)`. `DailyRewardItem/Bundle/Day/Bonus/Data` added to `GameObjects.cs`. 7 events. 14 new tests. |
| T50 | Protocol | ExtendedOpcode, TakeScreenshot, GameNews, Preset, PremiumTrigger, RuleViolationChannel/Cancel/Lock, ExperienceTracker parsers | Medium | 2 | ✅ Completed | `ParseExtendedOpcode(0x32→U8+str)`, `ParseTakeScreenshot(0x75→U8)`, `ParseGameNews(0x98→U32+U8)`, `ParsePreset(0x9D→U32)`, `ParsePremiumTrigger(0x9E→U8+U8[])`, `ParseRuleViolationChannel(0xAE→U16)`, `ParseExperienceTracker(0xAF@proto≥1200→2×I64)`, `ParseRuleViolationCancel(0xB0→str)`, `ParseRuleViolationLock(0xB1@proto<1310→no payload)`. 9 events. 18 new tests. |
| T51 | Protocol | SupplyStash, SpecialContainer, PartyAnalyzer, AttachedPaperdoll, DetachPaperdoll, Features, WeaponProficiencyExperience, PassiveCooldown, BosstiaryData, ClientCheck parsers | Medium | 2 | ✅ Completed | `ParseSupplyStash(0x29→U16+loop+U16)`, `ParseSpecialContainer(0x2A→U8+U8)`, `ParsePartyAnalyzer(0x2B→complex)`, `ParseAttachedPaperdoll(0x3C→U32+U16+6×U8+str)`, `ParseDetachPaperdoll(0x3D→U32+U8+U16)`, `ParseFeatures(0x43→U16+loop)`, `ParseWeaponProficiencyExperience(0x5C→U16+U32+U8)`, `ParsePassiveCooldown(0x5E→branches)`, `ParseBosstiaryData(0x61→18×U16)`, `ParseClientCheck(0x63→U32+loop)`. 9 events. 17 new tests. |
| T52 | Protocol | BosstiaryInfo, BosstiarySlots, BosstiaryCooldownTimer, BestiaryEntryChanged, UpdateImpactTracker, ItemsPrice, UpdateSupplyTracker, UpdateLootTracker, QuestTracker, KillTracker, ItemInfo, PlayerInventory parsers | Medium | 2 | ✅ Completed | 12 new parse methods. `BosstiaryEntry/BosstiarySlot/BosstiarySlotsData` added. `ProtocolVersion` property added. `ReadOutfit` gains `parseMount` param. 23 new tests. |
| T53 | Protocol | BrowseForgeHistory, BlessDialog, BestiaryTracker, TaskHuntingBasicData, TaskHuntingData, MonkData, CyclopediaHouseAuctionMessage, WeaponProficiencyInfo2, ChooseOutfit, BestiaryCharmsData parsers | Medium | 2 | ✅ Completed | 10 parse methods. `ForgeHistoryEntry/BlessData/BlessLogEntry/BlessDialogData/CharmData/BestiaryCharmsData/OutfitEntry/MountEntry/FamiliarEntry/OutfitWindowData` added. 9 events. 21 new tests. |
| T54 | Protocol | CyclopediaItemDetail, ItemClasses, CyclopediaHousesInfo, CyclopediaHouseList, RequestPurchaseData, ShowDescription, CloseImbuementWindow, ServerError parsers | Medium | 2 | ✅ Completed | 8 parse methods. `ForgeClassTierEntry/ForgeClassEntry/CyclopediaHouseState/CyclopediaHouseEntry` added. 8 events. 14 new tests. |
| T55 | Protocol | LoginChallenge, CyclopediaCharacterInfo (16 sub-types) parsers | Medium | 2 | ✅ Completed | `ParseLoginChallenge(0x1F→U32+U8+[U8@≥1405])`, `ParseCyclopediaCharacterInfo(0xDA→U8 type dispatch)` with 15 sub-type branches. 13 data records added to `GameObjects.cs`. 16 events. 14 new tests. |
| T56 | Protocol | Multi-version parser branches: AnimatedText/RemoveMagicEffect, Anthem/DistanceMissile, CreatureMark/ItemClasses, Trappers/OpenForge, RuleViolationLock/Highscores | Medium | 2 | ✅ Completed | `_enabledFeatures` HashSet + `HasFeature` added to `ProtocolGame`. 5 opcode handlers each branching on `ProtocolVersion` or `HasFeature`. `ForgeItemInfo/ForgeTransferData/ForgeOpenData/HighscoreEntry/HighscoresData` added. 4 new events. 10 new tests. |
| T57 | Protocol | Party/own-channel/outfit/typing send methods | Medium | 2 | ✅ Completed | `SendInviteToParty/JoinParty/RevokeInvitation/PassLeadership/LeaveParty/ShareExperience(0xA3-0xA8)`, `SendPartyAnalyzerAction(0x2B)`, `SendOpenOwnChannel/InviteToOwnChannel/ExcludeFromOwnChannel(0xAA-0xAC)`, `SendRequestOutfit(0xD2)`, `SendChangeOutfit(0xD3)`, `SendMountStatus(0xD4)`, `SendTyping(0x38)`. `FamiliarId` added to `Outfit.cs`. 14 new `GameClientPacket` values. 14 new tests (1868 total). |
| T58 | Protocol | SendWrapItem send method + WrapItemRequested event + wrapItem Lua binding | Medium | 2 | ✅ Completed | `SendWrapItem(0x8B→pos+U16+U8)` added to `ProtocolGameSend.cs`. `WrapItemRequested` event + `WrapItem` method added to `Game.cs`. `wrapItem` method added to `LuaGameProxy`. 1 new test (1869 total). |
| T59 | Protocol | VIP-edit + EquipItem + misc utility + bug/debug report send methods | Medium | 2 | ✅ Completed | 9 new `GameClientPacket` enum values: `GmTeleport=0x73`, `EquipItem=0x77`, `RefreshContainer=0xCA`, `RequestBless=0xCF`, `RequestTrackerQuestLog=0xD0`, `EditVip=0xDE`, `EditVipGroups=0xDF`, `BugReport=0xE6`, `DebugReport=0xE8`. 10 new send methods in `ProtocolGameSend.cs`: `SendGmTeleport`, `SendEquipItemWithTier`, `SendEquipItemWithCountOrSubType`, `SendRefreshContainer`, `SendRequestBless`, `SendRequestTrackerQuestLog`, `SendEditVip`, `SendEditVipGroups`, `SendBugReport`, `SendDebugReport`. `SendEditVip` honours `HasFeature(96)` (GameVipGroups) to append group IDs. `SendEquipItemWithCountOrSubType` honours `HasFeature(104)` (GameCountU16) for U16 vs U8 subtype. `SendBugReport` writes category byte 3 when `ProtocolVersion > 1000`. `SendEditVipGroups` silently no-ops on unknown action. 13 new tests (1882 total). |
